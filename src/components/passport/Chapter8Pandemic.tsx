    ...
    <div className="space-y-6">
      <div className="panel p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="panel p-4 space-y-3 bg-black/20">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-[10px] uppercase tracking-wider text-muted font-bold">Vaccine Name *</label>
                <input type="text" value={newVax.vaccine} onChange={(e) => setNewVax({ ...newVax, vaccine: e.target.value })} placeholder="e.g. Pfizer COVID-19" className="input-base w-full text-sm" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] uppercase tracking-wider text-muted font-bold">Date *</label>
                <input type="date" value={newVax.date} onChange={(e) => setNewVax({ ...newVax, date: e.target.value })} className="input-base w-full text-sm font-mono" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] uppercase tracking-wider text-muted font-bold">Dose #</label>
                <input type="number" min="1" value={newVax.dose} onChange={(e) => setNewVax({ ...newVax, dose: parseInt(e.target.value) || 1 })} className="input-base w-full text-sm font-mono" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] uppercase tracking-wider text-muted font-bold">Location</label>
                <input type="text" value={newVax.location} onChange={(e) => setNewVax({ ...newVax, location: e.target.value })} placeholder="Clinic name" className="input-base w-full text-sm" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] uppercase tracking-wider text-muted font-bold">Batch ID</label>
                <input type="text" value={newVax.batch} onChange={(e) => setNewVax({ ...newVax, batch: e.target.value })} placeholder="e.g. 4121Z01" className="input-base w-full text-sm font-mono" />
              </div>
            </div>
            <button onClick={addVax} className="btn-primary w-full text-sm flex items-center justify-center gap-2 py-3 mt-2">
          ...
          <div className="space-y-2">
            {vaccinations.map((v, i) => (
              <div key={i} className="flex items-center justify-between p-3 panel bg-black/10">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-full bg-teal/10 grid place-items-center font-mono font-bold text-teal text-xs">
                    {v.dose}
                  </div>
                  <div>
                    <div className="text-sm font-bold">{v.vaccine}</div>
                    <div className="text-[10px] text-muted font-mono">{v.date}{v.location ? ` · ${v.location}` : ''}</div>
                  </div>
                </div>
                <button onClick={() => updatePassportData({ vaccinations: vaccinations.filter((_, j) => j !== i) })} className="text-muted hover:text-red transition-colors" style={{ hover: { color: 'var(--red)' } }}>
                  <X className="h-4 w-4" />
                </button>
              </div>
            ))}
          ...
          <div className="panel p-4 space-y-3 bg-black/20">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <input type="date" value={newTest.date} onChange={(e) => setNewTest({ ...newTest, date: e.target.value })} className="input-base w-full text-sm font-mono" />
              </div>
              <div className="space-y-1.5">
                  ...
                  {['negative', 'positive'].map((r) => (
                    <button
                      ...
                      className={`py-2 rounded-md text-[10px] font-bold uppercase transition-all ${
                        newTest.result === r 
                          ? (r === 'positive' ? 'bg-red text-white' : 'bg-teal text-white') 
            ...
            <div className="space-y-1.5">
              ...
              <input type="text" value={newTest.type} onChange={(e) => setNewTest({ ...newTest, type: e.target.value })} placeholder="e.g. PCR, Rapid Antigen" className="input-base w-full text-sm" />
            </div>
            <button onClick={addTest} className="btn-primary w-full text-sm flex items-center justify-center gap-2 py-3 mt-2">
